...
- **智能体模板系统（专家代理蓝图市场）** - 新增功能
- 配置加载与热重载、审计与计量、沙箱与安全策略集成

本文件面向开发者与运维人员，系统化阐述 OpenFangKernel 的设计理念、Registry 的注册机制、Scheduler 的配额策略、Auth 的认证授权流程，以及**新增的 AgentTemplateRegistry 模板系统**，并给出与其他模块的交互关系、关键配置参数、错误处理与性能优化建议。
...
AT["AgentTemplateRegistry<br/>agent_templates"]
end
K --> R
K --> S
K --> A
...
**图表来源**
- [kernel.rs:115-125](file://crates/openfang-kernel/src/kernel.rs#L115-L125)
...
participant Templates as "AgentTemplateRegistry"
...
Kernel->>Templates : 初始化模板注册表
Templates->>Templates : 加载捆绑模板
Templates->>Templates : 加载用户安装模板
...
### AgentTemplateRegistry：智能体模板系统
**新增功能** - 本节介绍智能体模板系统的设计与实现

- 设计理念
  - 提供专家代理蓝图市场，支持快速创建标准化智能体
  - 集成捆绑模板（编译时嵌入）与用户安装模板（运行时加载）
  - 支持模板需求检查、设置管理、实例化等功能
  - 通过 API 提供完整的模板管理接口

- 数据结构
  - AgentTemplate：模板定义，包含基本信息、要求、设置、代理配置等
  - InstalledTemplate：已安装模板，包含模板定义和元数据
  - 模板目录：用户安装模板的存储位置

- 核心能力
  - **捆绑模板加载**：编译时嵌入的专家模板（如 Oh My OpenCode、Superpower、Security Expert 等）
  - **用户安装模板**：从目录加载用户自定义模板
  - **模板实例化**：将模板转换为完整的 AgentManifest
  - **需求检查**：验证模板运行所需的系统要求
  - **设置管理**：提供模板配置选项的状态检查
  - **模板安装/卸载**：支持从内容或目录安装模板

- 模板类型
  - **Bundled**：编译时嵌入的模板，无法卸载
  - **UserInstalled**：用户安装的模板，可启用/禁用和卸载
  - **Marketplace**：从市场下载的模板（预留支持）

- 模板要求检查
  - Binary：检查系统二进制文件是否存在
  - EnvVar：检查环境变量是否设置
  - ApiKey：检查 API 密钥环境变量是否设置
...
+get_installed(id) Option~InstalledTemplate~
+exists(id) bool
...
+install_from_content(toml, soul, bootstrap) AgentTemplateResult~AgentTemplate~
+uninstall(id) AgentTemplateResult~()~
+check_requirements(id) AgentTemplateResult~TemplateReadiness~
+get_settings_status(id) AgentTemplateResult~Vec~SettingStatus~~
+count() usize
+count_enabled() usize
}
class AgentTemplate {
...
+name : String
+description : String
+requires : Vec~TemplateRequirement~
...
+instantiate(settings) AgentManifest
}
...
- [lib.rs:248-394](file://crates/openfang-agents/src/lib.rs#L248-L394)
...
- **智能体模板系统故障排除**
  - 模板未显示：检查模板目录权限和文件完整性
  - 模板实例化失败：检查模板需求检查结果，确保所有必需要求得到满足
  - API 调用失败：确认模板 ID 正确性和模板启用状态
  - 捆绑模板缺失：重新构建项目以确保模板正确嵌入
...
## 结论
OpenFang 内核系统以模块化与高并发为核心设计原则，通过统一的 OpenFangKernel 聚合注册、调度、认证、事件、工作流等子系统，形成完整的智能体操作系统中枢。**新增的智能体模板系统进一步增强了系统的可扩展性和易用性**，通过专家代理蓝图市场，用户可以快速创建标准化的智能体，而无需从零开始配置。其配置加载、驱动链回退、RBAC 授权、事件总线以及模板系统等机制共同保障了系统的稳定性、可扩展性与安全性。建议在生产环境中结合配额与审计、触发器与工作流、模板系统，实现更高级别的自动化与可观测性。
...
- **智能体模板系统使用**
  - **模板注册表初始化**：[kernel.rs:795-811](file://crates/openfang-kernel/src/kernel.rs#L795-L811)
  - **模板列表获取**：[routes.rs:11225-11249](file://crates/openfang-api/src/routes.rs#L11225-L11249)
  - **模板实例化**：[routes.rs:11293-11351](file://crates/openfang-api/src/routes.rs#L11293-L11351)
- 集成测试（端到端）
...
- **智能体模板系统配置**
  - **模板目录**：`agent-templates` 子目录位于 HOME 目录下
  - **捆绑模板加载**：启动时自动加载编译时嵌入的模板
  - **用户模板加载**：启动时尝试加载用户安装的模板

### 智能体模板系统 API 参考
- **GET /api/agent-templates** - 列出所有可用模板
- **GET /api/agent-templates/{id}** - 获取单个模板详情
- **POST /api/agent-templates/{id}/instantiate** - 从模板创建智能体
- **GET /api/agent-templates/{id}/requirements** - 检查模板需求
- **GET /api/agent-templates/{id}/settings** - 获取模板设置状态
- **POST /api/agent-templates/{id}/enable** - 启用模板
- **POST /api/agent-templates/{id}/disable** - 禁用模板
- **DELETE /api/agent-templates/{id}** - 卸载模板

**章节来源**
- [routes.rs:11225-11443](file://crates/openfang-api/src/routes.rs#L11225-L11443)