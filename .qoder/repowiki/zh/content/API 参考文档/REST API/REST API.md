...
## 更新摘要
**变更内容**
- 新增了完整的智能体模板管理 API 端点系统，包括 /api/agent-templates 相关的完整路由
- 添加了模板列表、详情、实例化、要求检查、启用/禁用、卸载等功能
- 更新了端点清单，增加了 8 个新的模板管理端点
- 保持其他所有文档内容的准确性
...
- 智能体模板管理：新增的模板注册表系统，支持模板的加载、验证、实例化和管理。
...
- 其他常用类型：技能安装/卸载、代理更新、模式设置等。

**章节来源**
- [crates/openfang-api/src/types.rs:5-110](file://crates/openfang-api/src/types.rs#L5-L110)

### 智能体模板管理系统
- 模板注册表：支持内置模板和用户安装模板的统一管理
- 模板要求检查：自动检测二进制、环境变量、API 密钥等依赖
- 模板设置状态：提供可用性检查和选项状态
- 模板实例化：将模板转换为可运行的智能体清单
- 模板生命周期：支持启用/禁用、安装、卸载等操作

**章节来源**
- [crates/openfang-agents/src/registry.rs:14-229](file://crates/openfang-agents/src/registry.rs#L14-L229)
- [crates/openfang-agents/src/lib.rs:247-554](file://crates/openfang-agents/src/lib.rs#L247-L554)

## 依赖关系分析
- openfang-api 依赖 openfang-kernel、openfang-runtime、openfang-memory、openfang-channels、openfang-skills、openfang-hands、openfang-extensions、openfang-agents 等子 crate。
...
OA --> OAgs["openfang-agents<br/>模板管理"]
```
...
- GCRA 速率限制
  - 每 IP 每分钟 500 令牌。
  - 操作成本示例：健康检查 1、列出工具/技能/代理 2、状态/版本 1、代理列表 2、用量查询 3、审计/市场 5、创建代理 50、消息 30、运行工作流 100、更新代理 10。
- 日志与追踪
...
## 结论
OpenFang 的 REST API 以清晰的路由分层、完善的认证与安全策略、成本感知的速率限制、以及丰富的实时通信能力，支撑从智能体生命周期管理到工作流编排、渠道集成与技能市场的完整场景。新增的智能体模板管理系统进一步增强了平台的可扩展性和易用性，为用户提供了一套完整的模板管理解决方案。建议客户端在生产环境始终启用 API Key 认证与 HTTPS，并结合速率限制策略与合理的重试退避机制。
...
- 智能体模板管理
  ...
  - GET /api/agent-templates/{id}/requirements
  - GET /api/agent-templates/{id}/settings
  - POST /api/agent-templates/{id}/enable
  - POST /api/agent-templates/{id}/disable
  - DELETE /api/agent-templates/{id}
  - 示例：模板列表、详情查询、实例化创建、要求检查、设置状态、启用/禁用、卸载
  - 认证：需认证
...
- [crates/openfang-api/src/server.rs:121-722](file://crates/openfang-api/src/server.rs#L121-L722)
- [crates/openfang-api/src/types.rs:5-110](file://crates/openfang-api/src/types.rs#L5-L110)
- [crates/openfang-api/src/routes.rs:11224-11443](file://crates/openfang-api/src/routes.rs#L11224-L11443)
- [crates/openfang-agents/src/registry.rs:14-229](file://crates/openfang-agents/src/registry.rs#L14-L229)
- [crates/openfang-agents/src/lib.rs:247-554](file://crates/openfang-agents/src/lib.rs#L247-L554)